...
echo  "<option  value = ''>เลือก</option>" ;
$sql = "select  * from person_main where status='0' order by name,position_code";
$dbquery = mysqli_query($connect,$sql);
while ($result = mysqli_fetch_array($dbquery))
   {
...
//ส่วนบันทึกข้อมูล
if($index==4){
	$sqlrow = "select * from system_module_admin where person_id='$_POST[person_id]' AND module='$_POST[module]'";	
	$dbqueryrow = mysqli_query($connect,$sqlrow);
	$result_numrow = mysqli_num_rows($dbqueryrow);
	if($result_numrow > 0){
		echo "<script>alert('มีรายชื่อนี้เป็นผู้ดูแลระบบงานย่อย $_POST[module] นี้แล้ว กรุณาเปลี่ยนใหม่');</script>";
		}else{
$rec_date = date("Y-m-d");
...
echo "<td><div align='left'><Select  name='person_id'  size='1'>";
echo  "<option  value = ''>เลือก</option>" ;
$sql = "select  * from person_main order by name,position_code";
$dbquery = mysqli_query($connect,$sql);
while ($result = mysqli_fetch_array($dbquery))
   {
		$person_id = $result['person_id'];
...
//ส่วนปรับปรุงข้อมูล
if ($index==6){
		$sqlrow2 = "select * from system_module_admin where person_id='$_POST[person_id]' AND module='$_POST[module]'";	
		$dbqueryrow2 = mysqli_query($connect,$sqlrow2);
		$result_numrow2 = mysqli_num_rows($dbqueryrow2);
		if($result_numrow2 > 0){
		echo "<script>alert('มีรายชื่อนี้เป็นผู้ดูแลระบบงานย่อย $_POST[module] นี้แล้ว กรุณาเปลี่ยนใหม่');</script>";
		}else{
$rec_date = date("Y-m-d");
...
echo "<Tr bgcolor='#E6E6E6'><Td  align='center'>ที่</Td><Td align='center'>ระบบงานย่อย(อังกฤษ)</Td><Td align='center'>ระบบงานย่อย(ไทย)</Td><Td  align='center'>ผู้ดูแลระบบ</Td><Td align='center'>ลบ</Td><Td align='center'>แก้ไข</Td></Tr>";
...
while ($result = mysqli_fetch_array($dbquery))
	{
		...
		echo "<Td align='left'>";
				if(isset($username_ar[$person_id])){
				echo $username_ar[$person_id];
				}
		echo "</Td>";
		}
