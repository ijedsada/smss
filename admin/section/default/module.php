...
if (is_dir($dir)) {
    if ($dh = opendir($dir)) {
        while (($file = readdir($dh)) !== false) {
                if (($file == '.')||($file == '..')||($file == 'defualt')||($file == 'alert')||($file == 'amssplus'))
                {
                }
               else  
                {
...
//ส่วนบันทึกข้อมูล
if($index==4){
	if($_POST['module']=="amssplus"){
	echo "<script>document.location.href='?file=module';</script>\n";
	exit();
	}
...
$module_num=mysqli_num_rows($dbquery_module);
	...
	exit();
			...
			if(isset($_POST['module'])){
				...
				$sql = "insert into system_module (id, module, module_desc, workgroup, module_active, web_link, url,where_work,module_order) values ('$row_number', '$_POST[module]', '$_POST[module_desc]','$_POST[workgroup]','1','$_POST[web_link]','$_POST[url]','$_POST[where_work]','0')";
				$dbquery = mysqli_query($connect,$sql);
			}
	}
} //end if index
...
echo "<Tr><Td align='right'>ประเภทระบบงาน&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td align='left'><Select name='web_link' size='1'>";
			...
			echo  "<option value=0>ระบบงานย่อยภายในSMSS</option>" ;
			echo  "<option value=1 selected>เชื่อมโยงไปโปรแกรมภายนอก</option>" ;
			}
			else{
			echo  "<option value=0 selected>ระบบงานย่อยภายในSMSS</option>" ;
