...
$sql = "select system_user.id, person_main.name, person_main.surname, person_main.person_id, system_user.username, system_user.userpass from system_user left join person_main on system_user.person_id=person_main.person_id order by person_main.name limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='60%' border='1' align='center' style='border-collapse: collapse'>";
echo "<Tr bgcolor='#E6E6E6'><Td  align='center' width='50'>ที่</Td><Td align='center'>Username</Td><Td  align='center'>ชื่อ-สกุล</Td><Td align='center'>สถานะรหัสผ่าน</Td><Td align='center' width='50'>Reset</Td></Tr>";
